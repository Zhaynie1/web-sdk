<script lang="ts">
	...
	const VIDEO_SRC = staticAssetUrl('assets/video/starpetal_grove_bg.mp4');
	const BASE_AMBIENCE_SRC = staticAssetUrl('assets/audio/starpetal_grove_ambience.m4a');
	const BONUS_AMBIENCE_SRC = staticAssetUrl('assets/audio/starpetal_bonus_ambience.m4a');
	...
	const isBaseGame = () => !stateGameDerived.isFreeSpinsFeatureActive();
	const isBonusGame = () => stateGameDerived.isFreeSpinsFeatureActive();
	const showBonusTint = $derived(stateGameDerived.isFreeSpinsFeatureActive());
	...
	const scheduleAmbienceLoopIteration = (
		...
		const crossfade = Math.min(
			AMBIENCE_LOOP_CROSSFADE_SEC,
			Math.max(0.2, buffer.duration * 0.2),
		);
	...
	const startAmbienceLoop = (
		...
		if (!controller || !controller.stopped || !audioContext || !buffer || !gainNode) {
			return;
		}
	...
	const stopBaseAmbience = () => {
		stopAmbienceLoop(baseAmbienceLoop);
	};

	const stopBonusAmbience = () => {
		stopAmbienceLoop(bonusAmbienceLoop);
	};

	const startBaseAmbience = () => {
		if (!isBaseGame()) return;
		if (!baseAmbienceLoop || !baseAmbienceBuffer) return;
		startAmbienceLoop(baseAmbienceLoop, baseAmbienceBuffer);
	};

	const startBonusAmbience = () => {
		if (!isBonusGame()) return;
		if (!bonusAmbienceLoop || !bonusAmbienceBuffer) return;
	...
	const enableAudio = async () => {
		...
		if (!baseAmbienceBuffer || !bonusAmbienceBuffer) {
			await initAmbience();
		}
