<script lang="ts">
	...
	// Live tumble-win breakdown shown in the left feature window during play —
	// running total + a row per cluster (symbol icon · mult · win). Ported from the
	// original's LeftMultiplierPanel, keeping its proxima-nova colour styling.
	type Props = {
		x?: number;
		y?: number;
		width?: number;
...
{#if hasContent}
	<Container x={props.x} y={props.y}>
		...
		{#each entries as entry, index}
			{@const rowY = -panelHeight * 0.5 + (runningTotal > 0 ? 40 : 18) + index * ROW_HEIGHT}
